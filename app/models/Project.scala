...
object Project {
    ...
    DB.withConnection { implicit connection =>
      SQL(
        """
          ...
          join project_member on project_member.user_email = users.email 
  ...
  def isMember(project: Long, user: String): Boolean = {
    DB.withConnection { implicit connection =>
      SQL(
        """
          select count(users.email) = 1 from users 
          join project_member on project_member.user_email = users.email 
          where project_member.project_id = {project} and users.email = {email}
        """
      ).on(
        'project -> project,
        'email -> user
